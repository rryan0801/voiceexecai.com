...
export default function SEODashboard({ website, onBack }) {
  ...
  const { data: audits } = useQuery({
    ...
    queryFn: async () => await seoApi.getAudits(website.id)
  });

  const { data: keywords } = useQuery({
    queryKey: ['keywords', website.id],
    queryFn: async () => await seoApi.getKeywords(website.id)
  });

  const { data: optimizations } = useQuery({
    queryKey: ['optimizations', website.id],
    queryFn: async () => await seoApi.getOptimizations(website.id)
  });

  const { data: results } = useQuery({
    queryKey: ['seo-results', website.id],
    queryFn: async () => await seoApi.getResults(website.id)
  });

  const auditMutation = useMutation({
    mutationFn: async () => await seoApi.runAudit(website.id),
    onSuccess: () => { queryClient.invalidateQueries(['seo-audits', website.id]); toast.success('SEO audit completed!'); },
    onError: (error) => toast.error('Audit failed: ' + error.message)
  });

  const applyFixesMutation = useMutation({
    mutationFn: async (autoApply) => await seoApi.applySEOFixes(website.id, autoApply ? 'all' : []),
    onSuccess: () => { queryClient.invalidateQueries(['optimizations', website.id]); toast.success('SEO fixes applied!'); }
  });

  const trackRankingsMutation = useMutation({
    mutationFn: async () => await seoApi.trackKeywords(website.id),
    onSuccess: () => { queryClient.invalidateQueries(['keywords', website.id]); toast.success('Rankings updated!'); }
  });
